...
//включение нам необходимых пинов и портов (лучше делать в 1 месте;
//8- настройка ножки в режим Pull_up/pull-down (кнопка)
  
  GPIOA->CRH &= 0xFFFFFF0F; //Инокдер. PА9-Кнопка)
...
//Указатель откуда ловить прерывания
  AFIO->EXTICR[2] |= AFIO_EXTICR3_EXTI9_PA; //PА9 EXTICR3 ОТСЧЕТ С 0 = ПОТОМ РЕАЛЬНЫЙ EXTICR
...
//Сброс флагов прерываний
  ...
  NVIC_EnableIRQ(EXTI15_10_IRQn); //Включить прерывания с 10-по 15 ножки
